                              ...
                              <div className="w-[45%] flex flex-col">
                                    {
                                          companies.length > 0 && (
                                                <Select onValueChange={selectChangeHandler}>
                                                      <SelectTrigger className="rounded-full w-1/2 mx-auto mt-5 py-5 text-xl font-medium">
                                                            <SelectValue placeholder="Select a Company" />
                                                      </SelectTrigger>
                                                      <SelectContent className="bg-white shadow-xl text-justify text-xl overflow-hidden">
                                                            <SelectGroup>
                                                                  {
                                                                        companies
                                                                              .filter(company => company?.website && company?.logo)
                                                                              .map((company) => {
                                                                                    return (
                                                                                          <SelectItem
                                                                                                value={company?.name?.toLowerCase()}
                                                                                                key={company._id}
                                                                                                className="border border-black mx-auto mt-1 p-2 bg-slate-100 hover:bg-slate-50 cursor-pointer rounded-3xl "
                                                                                          >
                                                                                                <div className="flex items-center gap-5 font-serif text-base">
                        ...
                        <div>
                              {
                                    loading ? <Button className="cursor-not-allowed my-5 flex items-center justify-center border border-black rounded-full bg-green-500 hover:bg-green-600 font-bold"><Loader2 className="mr-2 h-5 w-5 animate-spin" />&nbsp;Please wait...</Button>
                                          ...
                                          <div className="flex gap-10 my-5">
                                                ...
                                                <Button type="submit" disabled={!input.companyId} className="flex items-center justify-center border border-black rounded-full bg-green-500 hover:bg-green-600 font-bold"><Save />&nbsp;Submit</Button>
                                          </div>
                              }
                        </div>
                        {
